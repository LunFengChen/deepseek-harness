...
Compaction is split between `@x1a0f3n9/dsh-compaction`, which owns an abstract two-method service and shared types, and `@x1a0f3n9/dsh-compaction-basic`, which owns the only complete provider. Shipped configurations load only the basic package, and no production package independently consumes the Service Definition package except that provider.
...
Move the basic implementation into `@x1a0f3n9/dsh-compaction` and remove `@x1a0f3n9/dsh-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@x1a0f3n9/dsh-compaction-basic` and its workspace/package metadata are removed.
- `@x1a0f3n9/dsh-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.

...
xfdsh 只 remap `@x1a0f3n9/dsh-*` 库包。官方 CLI 名 `@x1a0f3n9/dsh` 保持原样。版本探测改在 `bowenliang123/dsh-context` 的 fork `LunFengChen/dsh-context@0.49.6` 里：先读 `@x1a0f3n9/dsh` / `@xfcodeai/dsh` 和 fork 库包，最后才看残留的官方 CLI。web-app bundle 预装这个 GitHub 包。
...
- 官方插件 import `@x1a0f3n9/dsh-session` 时仍会 remap 到本 fork。

...
具名目录 `list` 继承同一读取权限；[具名目录列举与文件夹打开](2026-09-15-named-directory-listing-and-folder-open.zh.md)拥有列举、文件夹窗口和插件围栏决策。`changes` 仍限于工作区，并通过后端的工作区包含判定过滤观察。
...
**把所有操作限制在工作区内。** 这会让预览采用比 Session 文件系统后端更窄的策略，阻止读取明确寻址的可读文件，并使位于外部资源旁的 HTML 无法渲染。变更观察仍限于工作区。

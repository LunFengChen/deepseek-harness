...
The runtime executable packages `@xfcodeai/dsh` and runs its ordinary command grammar. The Python client selects `--profile sdk` by default, forwards ordered absolute `--patch` paths, and may select another `dsh` executable or profile. The runnable minimal example selects the shipped `sdk-minimal` profile. The private `@xfcodeai/dsh-sdk-python-runtime` application package and checked-in runtime `cordis.yml` do not exist. JSON-RPC serving remains the `@xfcodeai/dsh-sdk-app` bundle and `@xfcodeai/dsh-sdk-jsonrpc-server` plugin, not a Python-owned boot path.

...
`@x1a0f3n9/dsh-tool-todo` 在其仅类型出口中声明 `TodoItem`，并通过 `@x1a0f3n9/dsh-session/types` 的声明合并加入 `todo/write`。包根入口和 `/client` 入口重新导出 `TodoItem`，使 host 与浏览器消费方共享同一处声明，而无需加载 todo 插件。

...
`JobRegistry` is the Service Definition in `@x1a0f3n9/dsh-jobs`; the process-local provider is `LocalJobRegistry` in `@x1a0f3n9/dsh-jobs-local` (the [task-registry contract Agent Note](../../archived/architecture/2026-07-26-job-registry-seam.md) records that split).

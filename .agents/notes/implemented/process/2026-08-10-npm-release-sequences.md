...
- **Re-running publish over an older artifact can move `latest` backwards.** Publication is decided per version, so a later first-time publish of an older dsh `rc` takes `latest` again. `release:dist-tag` can move `latest` without republishing. Vendor rehearsals still use `next`.

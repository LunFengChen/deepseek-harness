...
`SubagentRuntime` is the only public service. It exposes ordinary `start(name, request)`, Task-backed `startContinuable(spec)`, and intent-named `followup(...)`; provider resume dispatch remains private to its continuation manager. The standalone `@x1a0f3n9/dsh-subagent-control` package and `ctx.subagentControl` key are absent; the optional `@x1a0f3n9/dsh-tool-subagent-control` package injects `ctx.subagents` directly.

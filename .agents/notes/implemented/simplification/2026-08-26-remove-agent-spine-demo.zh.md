...
`@xfcodeai/dsh-agent-spine-demo` 的名称与位置将它描述为示例，但它实际暴露了带大型合并配置的公开组合插件。其唯一交付消费方是 `dsh-sdk-minimal`，其中一个插件配置行隐藏了必需的 agent（智能体）运行时，而该组合包的目的本是提供完整、显式的配置树。其他消费方全部是测试。该包还重复了已归 `dsh-base` 所有的组合策略，却没有提供可独立演进的能力。

...
| 包 | 角色 |
|---|---|
| `@x1a0f3n9/dsh-subagent` | 接口：`SubagentRuntime`（`ctx.subagents`）、`SubagentProvider`、`SubagentRun`、请求、结果、能力词汇、`subagent/*` 事件 |
| `@x1a0f3n9/dsh-subagent-spawn-in-process` | 实现：通过 `ctx.agents.create` 创建全新的进程内子 agent |
| `@x1a0f3n9/dsh-subagent-fork-in-process` | 实现：用父 agent 日志快照初始化的进程内子 agent |
| `@x1a0f3n9/dsh-subagent-acp` | 实现：作为 ACP 客户端驱动已配置的子进程 |
| `@x1a0f3n9/dsh-subagent-codex` | 实现：一次性官方 Codex app-server 进程 |
| `@x1a0f3n9/dsh-subagent-claude-code` | 实现：通过 Agent SDK 运行的一次性官方 Claude Code 进程 |
| `@x1a0f3n9/dsh-tool-subagent` | 消费方：基于 `ctx.subagents` 的面向模型的 `subagent` 工具 |

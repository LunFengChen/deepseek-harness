...
dshmarket `v1.44.1` 从当前选中的 profile bundle 读取 `dsh.bundle.plugins`，作为 `prebundled` 返回，并把这些名字（含去 scope 的别名）当作仅用于匹配的在场记录。profile 的 `dependencies` 仍是变更的真相来源：已安装页可以列出预装包，但不能提供卸载或更新。Web bundle 钉的是 `github:LunFengChen/dsh-market#v1.44.1`。故障转移队列同样走目录，钉 `github:LunFengChen/dsh-failover-queue#v0.1.3`。

...
**固定 `searchProvider: search-pool`。** 否决：固定会盖住顺序，也会让设置里的 Default 不再表示“使用独占允许列表”。插件加载时，把 `search-pool` 放在 web-app 覆盖顺序的第一位就足以独占选择。

**把插件挂进 `dsh-base`，让每个 profile 都带上。** 否决：xfdsh 预置插件属于 web-app 目录，这样才能在设置里关掉。headless 和 SDK 仍走 Perplexity → Exa → free。

**默认包含无需密钥的 Tavily。** 否决：那会在本地 HTML 一跳之前消耗共享匿名配额。

## Consequences

- xfdsh web 默认 `web_search` 跑 `search-pool`；缺少 Perplexity/Exa 密钥时，在该提供方内部落到 `free`，而不是让独占顺序失败。
- 关掉设置 → 插件 → Web search pool 后，会跳过 `search-pool`，改走 Perplexity → Exa → free。

...
The workflow package exposes browser-safe run and observation vocabulary through `@x1a0f3n9/dsh-workflow/types`; live `Agent` requests and control handles remain Host-only. `@x1a0f3n9/dsh-tool-workflow/types` owns the four Session events. Client code imports only these type faces, so the Host and Client TypeScript programs share the durable contract without merging Host Cordis context.

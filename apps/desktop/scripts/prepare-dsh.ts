...
function desktopRelease(): DesktopRelease {
  ...
  if (version !== dshVersion) {
    throw new Error(`desktop runtime: Electron ${version} must bind the same version of @x1a0f3n9/dsh, found ${dshVersion}`)
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    writeFileSync(join(DSH_OUTPUT_ROOT, 'package.json'), `${JSON.stringify({
      name: '@x1a0f3n9/dsh-desktop-runtime', private: true, version: release.version, type: 'module',

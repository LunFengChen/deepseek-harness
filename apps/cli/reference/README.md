...
```sh
xfdsh plugin --profile <name> add @xfcodeai/dsh-subagent-codex
xfdsh plugin --profile <name> add @xfcodeai/dsh-subagent-claude-code
xfdsh plugin --profile <name> add @xfcodeai/dsh-subagent-codex @xfcodeai/dsh-subagent-claude-code
xfdsh plugin --profile <name> remove @xfcodeai/dsh-subagent-codex
xfdsh plugin --profile <name> remove @xfcodeai/dsh-subagent-claude-code
```
...
Install external plugin bundles through `xfdsh plugin --profile <name> add <package-or-git-spec>`. The installed package owns its dependencies and contributes its declared `cordis.patch.yml` layer. Official bundles may keep declaring `@deepseek-ai/dsh-*`; the profile's `.pnpmfile.cjs` turns those dependency specs into aliases for the matching `@xfcodeai/dsh-*` packages while keeping the package import names. Run `xfdsh plugin --profile <name> install` once after upgrading an existing profile so pnpm rebuilds these aliases. If a matching fork package is unavailable, installation fails explicitly and no second official runtime is loaded. The CLI also ships `@xfcodeai/dsh-mcp-client` as a dependency for patch layers, but no MCP server is enabled by default because each server command is trusted executable code outside the agent sandbox.

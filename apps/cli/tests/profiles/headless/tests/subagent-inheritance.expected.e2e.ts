...
import { createMessage, createUserMessage, ReasoningEffortId } from '@x1a0f3n9/dsh-llm'
import { SessionSeq, SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@x1a0f3n9/dsh-session'
...
async function seedReadOnlyParent(root: string, cwd: string): Promise<void> {
  ...
  const events: SessionEvent[] = [
    ...
    { type: 'step/start', seq: SessionSeq(1), time: 11, data: { turn: 1, step: 1 } },
    {
      ...
      data: { turn: 1, step: 1, message: createMessage({ role: 'system', content: [], source: { kind: 'plugin', plugin: '@x1a0f3n9/dsh-system-prompt' } }) },
      surfaceOp: 'append',
    },
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@x1a0f3n9/dsh-system-prompt') return []

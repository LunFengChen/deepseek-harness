...
手动输入的模型在自己声明之前一律按纯文本对待，因为没有任何环节能去询问端点接受哪些模态。如果会话历史或本轮提示词包含图片，纯文本请求会在适配器分发前将它们替换为稳定的文本占位符；持久会话历史不变，文本提示词会继续执行。
...
全部开关、各自接受的取值，以及接受它们的协议，都列在[生成的 `dsh-llm-pi-ai` 配置参考](../../config-catalog.zh.md#xfcodeaidsh-llm-pi-ai)的 `PiAiCompatProfile` 之下——该参考派生自源码，因此不会落后于适配器实际接受的内容。
...
- **图片会从纯文本请求中省略**：精确模型路由未声明图片模态。请求会继续执行，并将历史图片和新准入的图片替换为稳定文本占位符；只有自定义端点确实提供图片能力时，才应设置 `input: [text, image]`。DeepSeek 自身的 chat-completions 路由仍是纯文本。
- **提供方拒绝了带图片的请求**：路由元数据高估了端点能力。请从授予它图片能力的那个列表中移除 `image`——可能是模型的 `input`，也可能是路由的 `defaultInput`——后续请求就会使用纯文本投影，而不会发送视觉数据。

## 进阶配置

自动生成的[插件配置目录](../../config-catalog.zh.md)列出每个插件的所有受支持字段与默认值；[`dsh-llm-pi-ai`](../../config-catalog.zh.md#xfcodeaidsh-llm-pi-ai) 就是本页所配置的那个提供方段落。[`dsh-llm-pi-ai`](../../../packages/llm/llm-pi-ai/README.zh.md) 和 [`dsh-llm-deepseek`](../../../packages/llm/llm-deepseek/README.zh.md) 参考文档负责直接 `settings.yaml` 配置、目录解析、推理控制、凭据与适配器错误。

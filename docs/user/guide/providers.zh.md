...
::: tip 表单刻意保持精简
模型页只开放让一条路由得以存在的字段：API 密钥、显示名称、API 地址、API 协议，以及每个模型的 ID、显示名称、上下文窗口、最大输出 token 数，以及它是否接受图片。其余所有字段——推理等级、请求兼容性开关、请求头、超时、重试策略——都在 `$DSH_HOME/settings.yaml` 中设置，也就是模型页写入的同一份文档。可以直接编辑它；浏览器与服务器在同一台机器时，也可以点击设置页顶部的**打开配置文件**打开它。适配器会在下一次请求时重新读取，无需重启任何东西。下面各小节介绍多数网关会用到的字段。
:::
...
因此自定义提供方下的视觉模型需要这份声明。在该模型的容量折叠区打开**支持图片**，页面会写入 `input: [text, image]`。同一字段仍可在 `$DSH_HOME/settings.yaml` 中设置：
...
- **图片在发送前被拒绝**：该模型未声明图片模态。请在该模型的容量折叠区打开**支持图片**（或在 `settings.yaml` 中写 `input: [text, image]`）；在 DeepSeek 自身的路由上，请从配置的目录中选择支持图片的条目（默认为 `deepseek-flash`），并确认网关提供该模型且支持图片输入。

...
The Web boot kernel creates the module system, prefetches `immediately` entries, mounts the vendored Cordis Loader, and creates immediately-tier entries before deferred ones. Cordis service injection determines activation; module graph order determines only whether synchronous imports can be materialized. Once `uiRenderer` exists, it hydrates the framework-free boot DOM and calls the sole context-level `renderSlot('root')` operation; remaining entries may still be arriving, and the root outlet waits for the first live registration. [Client Modules](client-modules.md) owns the graph, bundle route, cache revision, and loader details.

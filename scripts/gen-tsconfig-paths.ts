 ...
 * `@x1a0f3n9/dsh-*` listed 49 candidate globs and `@x1a0f3n9/dsh-*\/invariant`
 ...
 * @returns Every directory whose manifest names a `@x1a0f3n9/dsh-` package and that carries `src`.
 */
 ...
 * `@x1a0f3n9/dsh-<directory>`. Packages named after something other than
 ...
 *
 * @returns Declared names of every `@x1a0f3n9/dsh-` package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@(?:deepseek-ai|xfcodeai|x1a0f3n9)\/dsh-[^"/]+)":/gm)) {
    const key = match[1]
...
function handWrittenSpecifiers(text: string): Set<string> {
  ...
  const keys = new Set<string>()
  for (const match of outside.matchAll(/^\s*"(@(?:deepseek-ai|xfcodeai|x1a0f3n9)\/dsh-[^"]+)":/gm)) {
    const key = match[1]
    if (key !== undefined) keys.add(key)
  }
  return keys

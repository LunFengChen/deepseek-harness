...
import SqliteSessionQueryEngine from '@xfcodeai/dsh-session-query-sqlite'
import GoalService from '@xfcodeai/dsh-goal'
import SystemPrompt from '@xfcodeai/dsh-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@xfcodeai/dsh-tools'
import LocalBashExecutor from '@xfcodeai/dsh-bash-local'
import * as BashEnvPlugin from '@xfcodeai/dsh-shell-env'
...
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@xfcodeai/dsh-attachment'
import UserQuestionService from '@xfcodeai/dsh-user-questions'
import PlanModeController from '@xfcodeai/dsh-plan-mode'
import WebRuntime from '@xfcodeai/dsh-web'
import * as WebSearchExa from '@xfcodeai/dsh-web-search-exa'
import * as WebFetchLocal from '@xfcodeai/dsh-web-fetch-http'
import SubagentRuntime from '@xfcodeai/dsh-subagent'
import type { SubagentProvider } from '@xfcodeai/dsh-subagent'
import * as ToolSubagentControl from '@xfcodeai/dsh-tool-subagent-control'
import * as ToolSubagentListAgents from '@xfcodeai/dsh-tool-subagent-control/list-agents'
import SkillRegistry from '@xfcodeai/dsh-skill'
import * as SkillFileSystem from '@xfcodeai/dsh-skill-filesystem'
import LocalJobRegistry from '@xfcodeai/dsh-jobs-local'
import * as ToolAskUser from '@xfcodeai/dsh-tool-ask-user'
import * as ToolBash from '@xfcodeai/dsh-tool-bash'
import * as ToolPwsh from '@xfcodeai/dsh-tool-pwsh'
import * as ToolBashPersistent from '@xfcodeai/dsh-tool-bash-persistent'
import * as ToolPwshPersistent from '@xfcodeai/dsh-tool-pwsh-persistent'
import CordisHostRunner from '@xfcodeai/dsh-cordis-host-runner'
import * as ToolCordis from '@xfcodeai/dsh-tool-cordis'
import * as ToolFs from '@xfcodeai/dsh-tool-fs'
import * as ToolFsSearch from '@xfcodeai/dsh-tool-fs-search'
import * as ToolStrReplaceEditor from '@xfcodeai/dsh-tool-str-replace-editor'
...
import * as ToolSchedule from '@xfcodeai/dsh-schedule'
import Lsp from '@xfcodeai/dsh-lsp'
import * as ToolLsp from '@xfcodeai/dsh-tool-lsp'
import * as ToolSkill from '@xfcodeai/dsh-tool-skill'
import * as ToolSessionQuery from '@xfcodeai/dsh-tool-session-query'
import * as ToolTasks from '@xfcodeai/dsh-tool-jobs'
import type TeamService from '@xfcodeai/dsh-experimental-agent-team'
import * as ToolTeam from '@xfcodeai/dsh-experimental-tool-agent-team'
...
import VmWorkflowEngine from '@xfcodeai/dsh-workflow-worker-thread'
import * as ToolRalph from '@xfcodeai/dsh-tool-ralph'
import * as ToolWorkflow from '@xfcodeai/dsh-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    pkg: '@xfcodeai/dsh-tool-ask-user',
    dir: 'tool-ask-user',
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@xfcodeai/dsh-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@xfcodeai/dsh-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@xfcodeai/dsh-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `DSH_*` environment comes from `@xfcodeai/dsh-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@xfcodeai/dsh-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@xfcodeai/dsh-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or DSH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@xfcodeai/dsh-tool-bash-persistent',
  ...
  {
    pkg: '@xfcodeai/dsh-tool-pwsh-persistent',
  ...
  {
    pkg: '@xfcodeai/dsh-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@xfcodeai/dsh-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. The image tool is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@xfcodeai/dsh-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  {
    pkg: '@xfcodeai/dsh-tool-terminal',
    dir: 'tool-terminal',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@xfcodeai/dsh-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@xfcodeai/dsh-tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@xfcodeai/dsh-tool-session-query',
  ...
  {
    pkg: '@xfcodeai/dsh-tool-subagent',
    dir: 'tool-subagent',
    source: {
  ...
  {
    pkg: '@xfcodeai/dsh-tool-subagent-control',
  ...
  {
    pkg: '@xfcodeai/dsh-experimental-tool-agent-team',
    dir: 'tool-agent-team',
  ...
  {
    pkg: '@xfcodeai/dsh-tool-workflow',
    dir: 'tool-workflow',

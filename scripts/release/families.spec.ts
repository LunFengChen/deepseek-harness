...
describe('release families', () => {
  it('excludes private experimental packages from the dsh release', () => {
    ...
    expect(members.map(member => member.name)).not.toContain('@x1a0f3n9/dsh-experimental-agent-team')
  })
  ...
  it('refuses an order that would publish a consumer before a dependency it installs', () => {
    ...
    expect(() => { dsh.publishOrder(members) }).toThrow(/no publish order honours @[^/]+\/dsh-charlie -> @[^/]+\/dsh-alpha/)
  })

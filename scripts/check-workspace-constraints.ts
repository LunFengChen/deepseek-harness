...
const experimentalPackageNamePrefix = '@xfcodeai/dsh-experimental-'
...
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  ...
  '@xfcodeai/dsh-web-frontend': ['dist', '!dist/**/*.map', '!dist/preview.html', '!dist/preview'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@xfcodeai/dsh-client-ui-primitives': ['lib/**/*.css'],
  '@xfcodeai/dsh-client-web': ['lib/**/*.css'],
  '@xfcodeai/dsh-client-ui-theme': ['lib/styles'],
  // The CPython side ships as source .py files, published as-is rather than built.
  '@xfcodeai/dsh-experimental-code-runtime-python': ['py/**/*.py'],
  // The shipped preset compositions travel inside the roster package.
  '@xfcodeai/dsh-agent-presets': ['presets'],
  // The Web Host mounts the default-off settings owner independently of each
  // Agent-scoped delegation-tool instance.
  '@xfcodeai/dsh-tool-subagent': ['lib/model-selection-settings.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@xfcodeai/dsh-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  '@xfcodeai/dsh-skill-badge': ['assets'],
  ...
  '@xfcodeai/dsh-experimental-webworker-packer': ['bin.js', 'lib/repository-*.js'],
  '@xfcodeai/dsh-subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
export function checkWorkspaceManifest({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@xfcodeai/dsh-')) {

...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@xfcodeai/dsh-client-' + short,
...
describe('source package uses', () => {
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@xfcodeai/dsh-a/subpath'",
      "declare module '@xfcodeai/dsh-client-ui-slots' {}",
      "const load = () => import('@xfcodeai/dsh-b/remote')",
    ...
    expect([...collectRuntimeSourcePackageUses('feature.tsx', [
      "import type { A } from '@xfcodeai/dsh-a/subpath'",
      "declare module '@xfcodeai/dsh-client-ui-slots' {}",
      "const load = () => import('@xfcodeai/dsh-b')",
      'export const view = <div />',
    ].join('\n'))].sort()).toEqual([
      '@xfcodeai/dsh-b',
      'react',
...
describe('package modes', () => {
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@xfcodeai/dsh-client-bootstrap/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('module requests', () => {
  it('rejects runtime requests from one client feature package to another dynamic row', () => {
    const ui = declaration('ui', {
      external: ['@xfcodeai/dsh-client-slots/client'],
      runtimeSourceUses: {
        '@xfcodeai/dsh-client-slots': ['packages/client/ui/src/client/index.ts'],
      },
    })
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [ui, slots] }))).toEqual([
      ...
      + '"@xfcodeai/dsh-client-slots/client"; import shared types only or call an injected Cordis service',
    ])
  })

  it('rejects stale externals and accepts a runtime import outside client feature packages', () => {
    const gateway = {
      ...declaration('@xfcodeai/dsh-api-gateway'), manifest: 'packages/api/gateway/package.json',
    }
    const stale = { ...declaration('@xfcodeai/dsh-api-stale', {
      external: ['@xfcodeai/dsh-api-gateway/client'],
    }), manifest: 'packages/api/stale/package.json' }
    const live = { ...declaration('@xfcodeai/dsh-api-live', {
      external: ['@xfcodeai/dsh-api-gateway/client'],
      runtimeSourceUses: {
        '@xfcodeai/dsh-api-gateway': ['packages/api/live/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@xfcodeai/dsh-api-gateway/client': ['packages/api/live/src/client/index.ts'],
      },
    }), manifest: 'packages/api/live/package.json' }
    expect(collectClientPackageViolations(facts([], {
      declarations: [gateway, stale, live],
    }))).toEqual([
      stale.manifest + ': dsh.client.external "@xfcodeai/dsh-api-gateway/client"'
  ...
  it('requires the exact external subpath to be imported at runtime', () => {
    ...
    const subject = { ...declaration('@xfcodeai/dsh-api-session-controller', {
      external: ['@xfcodeai/dsh-api-gateway/client'],
      runtimeSourceUses: {
        '@xfcodeai/dsh-api-gateway': ['packages/api/session-controller/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@xfcodeai/dsh-api-gateway/remote': ['packages/api/session-controller/src/client/index.ts'],
      },
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [gateway, subject] }))).toEqual([
      subject.manifest + ': dsh.client.external "@xfcodeai/dsh-api-gateway/client"'
      + ' has no runtime import or re-export in production source; remove the stale declaration',
    ])
  })
  ...
  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@xfcodeai/dsh-client-ui', '@xfcodeai/dsh-missing', '@xfcodeai/dsh-missing'],
      inject: ['', '@xfcodeai/dsh-a', '@xfcodeai/dsh-a'],
    })
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = { ...declaration('@xfcodeai/dsh-api-a', {
      external: ['@xfcodeai/dsh-api-b'],
      inject: ['@xfcodeai/dsh-api-b'],
      runtimeSourceUses: { '@xfcodeai/dsh-api-b': ['packages/api/a/src/client.ts'] },
      runtimeSourceSpecifiers: { '@xfcodeai/dsh-api-b': ['packages/api/a/src/client.ts'] },
    }), manifest: 'packages/api/a/package.json' }
    const b = { ...declaration('@xfcodeai/dsh-api-b', {
      external: ['@xfcodeai/dsh-api-a'],
      inject: ['@xfcodeai/dsh-api-a'],
      runtimeSourceUses: { '@xfcodeai/dsh-api-a': ['packages/client/b/src/client.ts'] },
      runtimeSourceSpecifiers: { '@xfcodeai/dsh-api-a': ['packages/client/b/src/client.ts'] },
...
describe('manifest declarations', () => {
  ...
  it('fixes malformed declaration entries without changing dependency sections', () => {
    ...
    const subject = pkg('feature', {
      external: ['', 'react', '@xfcodeai/dsh-client-feature', '@xfcodeai/dsh-missing'],
      inject: ['', '@xfcodeai/dsh-agent', '@xfcodeai/dsh-agent'],
      sourceUses: {
        '@xfcodeai/dsh-agent': ['packages/client/feature/src/index.ts'],
        '@xfcodeai/dsh-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@xfcodeai/dsh-agent': 'workspace:*',
      },
      peerDependencies: {
        '@xfcodeai/dsh-client-ui-slots': 'workspace:^',
    ...
    expect(fixed.dsh.client).toMatchObject({
      external: ['@xfcodeai/dsh-missing'],
      inject: ['@xfcodeai/dsh-agent'],
    })

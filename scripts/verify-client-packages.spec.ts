...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@x1a0f3n9/dsh-client-' + short,
...
describe('source package uses', () => {
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@x1a0f3n9/dsh-a/subpath'",
      "declare module '@x1a0f3n9/dsh-client-ui-slots' {}",
      "const load = () => import('@x1a0f3n9/dsh-b/remote')",
    ...
    expect([...collectRuntimeSourcePackageUses('feature.tsx', [
      "import type { A } from '@x1a0f3n9/dsh-a/subpath'",
      "declare module '@x1a0f3n9/dsh-client-ui-slots' {}",
      "const load = () => import('@x1a0f3n9/dsh-b')",
      'export const view = <div />',
    ].join('\n'))].sort()).toEqual([
      '@x1a0f3n9/dsh-b',
      'react',
...
describe('package modes', () => {
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@x1a0f3n9/dsh-client-bootstrap/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('module requests', () => {
  it('rejects runtime requests from one client feature package to another dynamic row', () => {
    const ui = declaration('ui', {
      external: ['@x1a0f3n9/dsh-client-slots/client'],
      runtimeSourceUses: {
        '@x1a0f3n9/dsh-client-slots': ['packages/client/ui/src/client/index.ts'],
      },
    })
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [ui, slots] }))).toEqual([
      ...
      + '"@x1a0f3n9/dsh-client-slots/client"; import shared types only or call an injected Cordis service',
    ])
  })

  it('rejects stale externals and accepts a runtime import outside client feature packages', () => {
    const gateway = {
      ...declaration('@x1a0f3n9/dsh-api-gateway'), manifest: 'packages/api/gateway/package.json',
    }
    const stale = { ...declaration('@x1a0f3n9/dsh-api-stale', {
      external: ['@x1a0f3n9/dsh-api-gateway/client'],
    }), manifest: 'packages/api/stale/package.json' }
    const live = { ...declaration('@x1a0f3n9/dsh-api-live', {
      external: ['@x1a0f3n9/dsh-api-gateway/client'],
      runtimeSourceUses: {
        '@x1a0f3n9/dsh-api-gateway': ['packages/api/live/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@x1a0f3n9/dsh-api-gateway/client': ['packages/api/live/src/client/index.ts'],
      },
    }), manifest: 'packages/api/live/package.json' }
    expect(collectClientPackageViolations(facts([], {
      declarations: [gateway, stale, live],
    }))).toEqual([
      stale.manifest + ': dsh.client.external "@x1a0f3n9/dsh-api-gateway/client"'
  ...
  it('requires the exact external subpath to be imported at runtime', () => {
    ...
    const subject = { ...declaration('@x1a0f3n9/dsh-api-session-controller', {
      external: ['@x1a0f3n9/dsh-api-gateway/client'],
      runtimeSourceUses: {
        '@x1a0f3n9/dsh-api-gateway': ['packages/api/session-controller/src/client/index.ts'],
      },
      runtimeSourceSpecifiers: {
        '@x1a0f3n9/dsh-api-gateway/remote': ['packages/api/session-controller/src/client/index.ts'],
      },
    ...
    expect(collectClientPackageViolations(facts([], { declarations: [gateway, subject] }))).toEqual([
      subject.manifest + ': dsh.client.external "@x1a0f3n9/dsh-api-gateway/client"'
      + ' has no runtime import or re-export in production source; remove the stale declaration',
    ])
  })
  ...
  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@x1a0f3n9/dsh-client-ui', '@x1a0f3n9/dsh-missing', '@x1a0f3n9/dsh-missing'],
      inject: ['', '@x1a0f3n9/dsh-a', '@x1a0f3n9/dsh-a'],
    })
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = { ...declaration('@x1a0f3n9/dsh-api-a', {
      external: ['@x1a0f3n9/dsh-api-b'],
      inject: ['@x1a0f3n9/dsh-api-b'],
      runtimeSourceUses: { '@x1a0f3n9/dsh-api-b': ['packages/api/a/src/client.ts'] },
      runtimeSourceSpecifiers: { '@x1a0f3n9/dsh-api-b': ['packages/api/a/src/client.ts'] },
    }), manifest: 'packages/api/a/package.json' }
    const b = { ...declaration('@x1a0f3n9/dsh-api-b', {
      external: ['@x1a0f3n9/dsh-api-a'],
      inject: ['@x1a0f3n9/dsh-api-a'],
      runtimeSourceUses: { '@x1a0f3n9/dsh-api-a': ['packages/client/b/src/client.ts'] },
      runtimeSourceSpecifiers: { '@x1a0f3n9/dsh-api-a': ['packages/client/b/src/client.ts'] },
...
describe('manifest declarations', () => {
  ...
  it('fixes malformed declaration entries without changing dependency sections', () => {
    ...
    const subject = pkg('feature', {
      external: ['', 'react', '@x1a0f3n9/dsh-client-feature', '@x1a0f3n9/dsh-missing'],
      inject: ['', '@x1a0f3n9/dsh-agent', '@x1a0f3n9/dsh-agent'],
      sourceUses: {
        '@x1a0f3n9/dsh-agent': ['packages/client/feature/src/index.ts'],
        '@x1a0f3n9/dsh-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@x1a0f3n9/dsh-agent': 'workspace:*',
      },
      peerDependencies: {
        '@x1a0f3n9/dsh-client-ui-slots': 'workspace:^',
    ...
    expect(fixed.dsh.client).toMatchObject({
      external: ['@x1a0f3n9/dsh-missing'],
      inject: ['@x1a0f3n9/dsh-agent'],
    })

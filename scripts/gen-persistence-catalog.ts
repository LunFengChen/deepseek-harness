...
const SESSION_PACKAGE = '@xfcodeai/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@xfcodeai/dsh-session/types'
 ...
 * top-level declaration (in `@xfcodeai/dsh-session`) and any declaration
 * merge inside a `declare module '@xfcodeai/dsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @xfcodeai/dsh-session/known-event-types',
    ' */',
    '',
    '/**',

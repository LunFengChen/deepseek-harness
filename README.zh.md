...
| 故障转移队列 | 预装且可关闭：`github:LunFengChen/dsh-failover-queue#v0.1.9`。 | 输入框芯片，加上 Settings → 故障转移 里的 P1/P2/P3 编辑。在 `llm-retry` 用尽后切下一档，或在 `AUTH` / `RATE_LIMIT` / `NO_ADAPTER` 时立即切换。 |

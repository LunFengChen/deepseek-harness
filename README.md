...
| Hindsight memory | Preinstalled and off by default: `@vectorize-io/hindsight-coding-agents@0.5.2`. | Enable it from Settings → Plugins. xfdsh defaults to a local daemon; cloud or a self-hosted URL is optional in `~/.hindsight/coding-agent.json`. Local recall needs `uv` and an extraction LLM key. |

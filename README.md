...
| Plugin market | Preinstalled, disableable plugin `github:LunFengChen/dsh-market#v1.44.1`. Official `@deepseek-ai/dsh-*` plugins remap into this runtime. | Community plugins install with `xfdsh plugin --profile web add`. |
| Plugin catalog | Optional-plugin cards show the package version. The package name links to the GitHub pin. | Settings → Plugins does not add a separate author byline; `@scope/name` already names the owner. |
...
| Failover queue | Preinstalled, disableable plugin `github:LunFengChen/dsh-failover-queue#v0.1.3`. | The composer chip shows `故障转移：P1 provider/model` and fails over after `llm-retry`, or immediately on `AUTH` / `RATE_LIMIT` / `NO_ADAPTER`. |

...
| Plugin market | Preinstalled, disableable plugin `github:LunFengChen/dsh-market#v1.44.2`. Official `@deepseek-ai/dsh-*` plugins remap into this runtime. | Community plugins install with `xfdsh plugin --profile web add`. |
| xfdsh preset plugins | Prebundled fork plugins live on the **xfdsh preset plugins** tab. Cards show the package version; the package name links to the GitHub pin. Plugin list keeps session/global inventory and omits those packages. | Settings → Plugins → xfdsh preset plugins enables or disables them without mixing them into Plugin list. |
...
| Failover queue | Preinstalled, disableable plugin `github:LunFengChen/dsh-failover-queue#v0.1.4`. | The composer chip shows `故障转移：P1 provider/model` and fails over after `llm-retry`, or immediately on `AUTH` / `RATE_LIMIT` / `NO_ADAPTER`. |

...
import { createAssistantMessage, createSystemMessage, createUserMessage, createToolResultMessage, ToolCallId } from '@x1a0f3n9/dsh-llm'
import type { StreamChunk } from '@x1a0f3n9/dsh-llm'
import { AssistantStreamAccumulator } from '@x1a0f3n9/dsh-llm/assistant-stream'
import { Session, SessionId, SESSION_FORMAT_VERSION } from '@x1a0f3n9/dsh-session'
...
export function syntheticHistory(): string {
  ...
  for (let turn = 1; turn <= HISTORY_TURNS; turn++) {
    ...
    if (turn === 1) session.append('system/message', {
      turn, step: 1, message: createSystemMessage('', '@x1a0f3n9/dsh-system-prompt'),
    }, { surfaceOp: 'append' })

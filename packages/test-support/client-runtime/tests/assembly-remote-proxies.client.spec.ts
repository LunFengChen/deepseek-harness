...
import { RemoteMock, frames, ok, openStream } from '@x1a0f3n9/dsh-remote-mock'
...
const API_ROSTER = webApp.closure(['@x1a0f3n9/dsh-api-remotes'])
...
describe('remote proxies over a booted client', () => {
  ...
  it('drops the api-remotes row and answers ctx.remote.<ns>.<method> with the mock value, logging positional args', async () => {
    ...
    expect([...client.ctx.loader.entries()].map(entry => entry.options.name)).not.toContain('@x1a0f3n9/dsh-api-remotes')

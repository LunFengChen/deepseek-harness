...
vi.mock('@x1a0f3n9/dsh-atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@x1a0f3n9/dsh-atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@x1a0f3n9/dsh-atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }

...
import * as GoalSessionInvariant from '@xfcodeai/dsh-goal-round-driver/invariant'
import { renderGoalRoundPrompt } from '@xfcodeai/dsh-goal-round-driver'
import InvariantRegistry, { InvariantError } from '@xfcodeai/dsh-invariants'
import SessionStore, { SessionId, type Session } from '@xfcodeai/dsh-session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@xfcodeai/dsh-goal-round-driver',
    }))
  })

...
describe('search-pool egress', () => {
  it('goes through the proxy for a credentialed adapter', async () => {
    expect(await observe(() => searchWithAdapter(
      { kind: 'tavily', apiKey: 'probe', baseURL: 'http://tavily-probe.invalid' },
      { query: 'probe' },
    ))).toEqual(['REQ http://tavily-probe.invalid/search'])
  })
})

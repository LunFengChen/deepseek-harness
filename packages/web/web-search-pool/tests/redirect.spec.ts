...
import { searchWithAdapter } from '@x1a0f3n9/dsh-web-search-pool'
...
describe('search-pool credentialed adapter redirect policy', () => {
  it.each([301, 302, 303, 307, 308])('rejects HTTP %i before contacting Location', async (status) => {
    targetRequests.length = 0
    await expect(searchWithAdapter(
      { kind: 'tavily', apiKey: TEST_API_KEY, baseURL: `${redirectOrigin}/${status}` },
      { query: TEST_QUERY },

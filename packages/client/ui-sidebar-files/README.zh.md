...
- **类型**：`ctx.sidebarRightTabs.register(...)`，kind 为 `files`，id 为 `@x1a0f3n9/dsh-client-ui-sidebar-files`，档位 `builtin`，没有 patterns，另有一个打开该类型的引导页入口（order 10，标题与描述取自 `sidebarFiles` 命名空间，图标是共享的文件夹图标）。
...
根是会话的工作目录，读自 `useSessions().byId[sessionId].cwd`，标题行里的拆分由 `@x1a0f3n9/dsh-util-workspace-path` 的 `pathPartsOf` 给出。每一层以绝对路径为键；子路径是父路径以 `/` 拼上条目名。一层在首次展开时经 `@x1a0f3n9/dsh-api-workspace-files` 命名空间的 `remote.workspaceFiles.list(sessionId, absolutePath)` 列出；适配器保留列表的条目与截断标志，丢弃其工作区相对路径。行序为目录优先，其后按自然序、不分大小写的名称排列；dotfiles 与其他条目一样显示。
...
- **只有一个根。**树以会话工作目录为根；本页没有办法浏览到它之上。其他入口的具名目录列举仍可指向 Session 文件系统允许读取的路径。

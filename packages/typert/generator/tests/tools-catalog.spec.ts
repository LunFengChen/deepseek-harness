...
import type { TypertContribution } from '@x1a0f3n9/dsh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@x1a0f3n9/dsh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    ...
    const artifact = new FaceModelEmitter(host).emit('@x1a0f3n9/dsh-tools')
    ...
    const record = ctx.typert.getPackage('@x1a0f3n9/dsh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@x1a0f3n9/dsh-tools', 'host')).toBeUndefined()
  })
})

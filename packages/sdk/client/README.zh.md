...
- **无捆绑运行时解析**——客户端解析同版本 `@xfcodeai/dsh` 包（或调用方提供的 `dshBin`）；打包可执行文件的发现留在 Python 侧，直到出现 TypeScript 发行版消费方。

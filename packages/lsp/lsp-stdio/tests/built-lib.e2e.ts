...
/**
 * Keyless built-artifact smoke: plain Node imports `@x1a0f3n9/dsh-lsp` and
 * `@x1a0f3n9/dsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@x1a0f3n9/dsh-lsp')
      const LspLocal = await import('@x1a0f3n9/dsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@x1a0f3n9/dsh-fs-local')

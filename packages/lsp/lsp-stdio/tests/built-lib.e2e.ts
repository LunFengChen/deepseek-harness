...
/**
 * Keyless built-artifact smoke: plain Node imports `@xfcodeai/dsh-lsp` and
 * `@xfcodeai/dsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@xfcodeai/dsh-lsp')
      const LspLocal = await import('@xfcodeai/dsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@xfcodeai/dsh-fs-local')

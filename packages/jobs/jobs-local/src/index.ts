 ...
 * @module @x1a0f3n9/dsh-jobs-local
 */

import { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import type { Agent } from '@x1a0f3n9/dsh-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@x1a0f3n9/dsh-scope'
import type { ScopeLayer } from '@x1a0f3n9/dsh-scope'
 ...
 * `@x1a0f3n9/dsh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @x1a0f3n9/dsh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @x1a0f3n9/dsh-agent)')
    }

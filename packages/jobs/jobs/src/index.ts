 ...
 * process-local registry lives in `@xfcodeai/dsh-jobs-local`.
 * @module @xfcodeai/dsh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@xfcodeai/dsh-jobs is the abstract job registry seam; load an implementation such as @xfcodeai/dsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }

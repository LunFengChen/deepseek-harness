...
import * as workspaceContext from '@xfcodeai/dsh-agent-instructions'
import LlmRuntime, { createUserMessage, ToolCallId, type Message, type StreamChunk } from '@xfcodeai/dsh-llm'
import SessionStore, { Session, SessionId, SessionSeq, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@xfcodeai/dsh-session'
import SessionProjectionRegistry from '@xfcodeai/dsh-session-projection'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@xfcodeai/dsh-agent'
import AgentLoop, { turnBoundaryProjectionDefinition } from '@xfcodeai/dsh-agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@xfcodeai/dsh-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import type {
  ToolExecution,
  ToolExecutionToken,
} from '@xfcodeai/dsh-tools'
import * as ToolFs from '@xfcodeai/dsh-tool-fs'
import {
  discoverBaselineInstructionFiles,
  loadBaselineInstructions,
  renderWorkspaceContext,
} from '@xfcodeai/dsh-agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  ...
  it('labels the default DSH home as ~/.dsh when HOME points at the configured default', async () => {
    ...
    try {
      ...
      const isolated = await import('@xfcodeai/dsh-agent-instructions')

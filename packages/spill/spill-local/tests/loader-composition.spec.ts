...
import LocalSpillStore, { sessionDir } from '@x1a0f3n9/dsh-spill-local'
  ...
  it('loads cleanupPeriodDays and prunes only expired session contents', async () => {
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier !== '@x1a0f3n9/dsh-spill-local') throw new Error(`unexpected Loader import: ${specifier}`)

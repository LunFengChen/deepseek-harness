...
import * as CommandInvariant from '@xfcodeai/dsh-commands/invariant'
import InvariantRegistry, { InvariantError } from '@xfcodeai/dsh-invariants'
import SessionStore, { SessionId, SessionSeq, type Session } from '@xfcodeai/dsh-session'
import { CommandId } from '@xfcodeai/dsh-commands'
...
describe('command lifecycle invariants', () => {
  ...
  it.each([-1, 1.5, 1])('rejects invalid or non-prior sourceEventSeq %s', async (sourceEventSeq) => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@xfcodeai/dsh-commands',
    }))
  })

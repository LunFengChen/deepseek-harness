...
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@x1a0f3n9/dsh-tools'
...
import type { ApprovalOutcome } from '@x1a0f3n9/dsh-user-approval'
...
import type { ShellProcessRead } from '@x1a0f3n9/dsh-shell'
...
describe('sandbox escalation through ctx.approval', () => {
  ...
  it.each(['workspace-write', 'danger-full-access'] as const)(
    ...
    async (requestedMode) => {
      const { ctx, bash } = await setupSandboxed()
      const result = await call(ctx, 'pwsh', { ...escalate, sandbox_permissions: requestedMode }, sandboxAgent('danger-full-access'))
      expect(result.isError).toBe(false)
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @x1a0f3n9/dsh-jobs and @x1a0f3n9/dsh-tool-jobs')
  })

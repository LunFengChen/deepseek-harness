...
/**
 * Scan a concatenated Zstandard file through bounded range reads.
 * @param size - current physical file size in bytes.
 * @param read - range reader that returns at most the requested bytes.
 * @param signal - optional cancellation signal.
 * @param maxFrames - optional number of complete frames to discover.
 * @returns complete frame ranges and an optional incomplete-final-frame start.
 */
export async function scanZstdFrameRanges(
  size: number,
  ...
  const readAt = async (position: number, length: number): Promise<Buffer | undefined> => {
    ...
    const value = await read(position, length)
    signal?.throwIfAborted()
  ...
  while (offset < size) {
    ...
    if (magic === undefined) return { frames, tornStart: start }
    if (magic.readUInt32LE(0) !== ZSTD_MAGIC) {
      throw new Error(`corrupt Zstandard session log: invalid frame magic at byte ${offset}`)
    }
    offset += 4
    const descriptorBuffer = await readAt(offset, 1)
    if (descriptorBuffer === undefined) return { frames, tornStart: start }
    const descriptor = descriptorBuffer[0] as number
    offset += 1
    if ((descriptor & 0x18) !== 0) {
      throw new Error(`corrupt Zstandard session log: reserved frame-header bit at byte ${offset - 1}`)
    }
    ...
    if (await readAt(offset, headerBytes) === undefined) return { frames, tornStart: start }
    offset += headerBytes

    for (;;) {
      const blockHeader = await readAt(offset, 3)
      if (blockHeader === undefined) return { frames, tornStart: start }
      offset += 3
      const value = blockHeader.readUIntLE(0, 3)
      const lastBlock = (value & 1) !== 0
      ...
      const payloadBytes = blockType === 0x01 ? 1 : blockSize
      if (offset + payloadBytes > size) return { frames, tornStart: start }
      offset += payloadBytes
      if (lastBlock) break
    }
    if (checksum) {
      if (await readAt(offset, 4) === undefined) return { frames, tornStart: start }
      offset += 4
    }
    frames.push({ start, end: offset })
    if (frames.length >= maxFrames) return { frames }

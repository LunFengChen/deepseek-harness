...
本组分为五个家族：持久存储（持久化 seam、后端、检查点策略）、恢复、投影、标题与遥测。每个包 README 负责各自的约定与配置。
...
| 包 | 职责 | ctx key |
|---|---|---|
| [`session-rewind/`](session-rewind/README.zh.md) | 回退模型可见的对话，并按持久化编辑前检查点可选恢复工作区文件 | 注册 `/rewind` 与 Web 回退操作 |

...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@x1a0f3n9/dsh-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@x1a0f3n9/dsh-tools'
import type { Agent } from '@x1a0f3n9/dsh-agent'
import { Session, SessionId } from '@x1a0f3n9/dsh-session'
import type { SessionEventMap } from '@x1a0f3n9/dsh-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@x1a0f3n9/dsh-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)

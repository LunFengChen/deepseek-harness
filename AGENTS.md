...
packages/    @xfcodeai/dsh-<pkg> workspaces at packages/<group>/<pkg>/
...
- Every npm package is `@xfcodeai/dsh-<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@deepseek-ai/cordis` is a peerDependency (+ dev) of every harness package.
